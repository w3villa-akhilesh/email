...
class S3Service:
    ...
    def upload_bytes(self, file_bytes: bytes, object_key: str, content_type: Optional[str] = None) -> str:
        """
        ...
        try:
            ...
            url = f"https://{self.bucket_name}.s3.{self.region_name}.amazonaws.com/{object_key}"
            logger.info(f"Successfully uploaded bytes to S3: {url}")
            return url
        except NoCredentialsError:
            logger.error("AWS credentials not available")
            raise Exception("AWS credentials not configured")
        except ClientError as e:
            logger.error(f"AWS S3 put_object failed: {str(e)}")
            raise Exception(f"S3 upload failed: {str(e)}")
        except Exception as e:
            logger.error(f"Unexpected error during S3 upload (bytes): {str(e)}")
            raise Exception(f"Upload failed: {str(e)}")
